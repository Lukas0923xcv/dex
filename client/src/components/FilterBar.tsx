...
import { Search, X, Check, RotateCcw, Sparkles, Flame, ArrowUpDown, Compass, SlidersHorizontal, ChevronDown, ChevronUp } from 'lucide-react';
...
export const FilterBar: React.FC<FilterBarProps> = ({
  ...
  // Count secondary filters for the mobile badge
  const secondaryFilterCount = useMemo(() => {
    let count = 0;
    if (filters.availability && filters.availability !== 'all') count++;
    if (filters.sortBy !== 'dexAsc') count++;
    if (filters.shinyOnly) count++;
    if (filters.shadowOnly) count++;
    if (showFormsControls && filters.showGenderTracking) count++;
    if (showFormsControls && filters.includeBaseInForms) count++;
    return count;
  }, [filters, showFormsControls]);

  return (
    <div className="bg-white dark:bg-slate-900/90 backdrop-blur border border-slate-200 dark:border-slate-800 rounded-2xl p-3 sm:p-4 shadow-sm dark:shadow-xl mb-4 sm:mb-6 space-y-3 sm:space-y-4 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-3 flex-wrap">
        ...
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full pl-10 pr-9 py-2 sm:py-2 bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/80 focus:border-blue-500 rounded-xl text-base sm:text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-blue-500 transition-all"
          />
          {filters.search && (
            <button
        ...
        {/* Mobile Quick Row: Status Tabs + Mobile Filter Toggle Button */}
        <div className="flex items-center justify-between gap-1.5 sm:hidden">
          {/* Status Quick Filter Chips */}
          <div className="flex items-center bg-slate-100 dark:bg-slate-800/80 p-0.5 rounded-xl border border-slate-200 dark:border-slate-700/80 flex-1">
            {(['all', 'caught', 'uncaught'] as StatusFilter[]).map((st) => (
              <button
                key={st}
                onClick={() => onFilterChange({ status: st })}
                className={`flex-1 py-1.5 rounded-lg text-xs font-bold capitalize transition-all active:scale-95 ${
                  filters.status === st
                    ? st === 'caught'
          ...
          {/* Toggle More Filters Button on Mobile */}
          <button
            type="button"
            onClick={() => setIsMobileFiltersOpen(prev => !prev)}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold border transition-all active:scale-95 shrink-0 cursor-pointer ${
              isMobileFiltersOpen || secondaryFilterCount > 0
          ...
          >
            ...
            {isMobileFiltersOpen ? <ChevronUp className="w-3 h-3 text-slate-400" /> : <ChevronDown className="w-3 h-3 text-slate-400" />}
          </button>
          ...
          {hasActiveFilters && (
            <button
              ...
              className="p-1.5 text-xs text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900/50 rounded-xl transition-colors active:scale-95 shrink-0 cursor-pointer"
        ...
        {/* Desktop Status Quick Filter Chips */}
        <div className="hidden sm:flex items-center bg-slate-100 dark:bg-slate-800/80 p-1 rounded-xl border border-slate-200 dark:border-slate-700/80 shrink-0">
          {(['all', 'caught', 'uncaught'] as StatusFilter[]).map((st) => (
            <button
              key={st}
              onClick={() => onFilterChange({ status: st })}
        ...
        {/* Secondary Filter Controls (Always on desktop, Collapsible on phone) */}
        <div className={`${isMobileFiltersOpen ? 'flex' : 'hidden sm:flex'} flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100 dark:border-slate-800`}>
          {/* Sort Dropdown */}
          ...
          {mode !== 'shiny' && (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all shrink-0 select-none cursor-pointer active:scale-95 ${
                filters.shinyOnly
          ...
          {mode !== 'shadow' && (
            <button
              ...
              title="Show only Pokémon that have a shadow version in Pokémon GO"
            >
          ...
          {hasActiveFilters && (
            <button
              onClick={onClearFilters}
              className="hidden sm:inline-flex px-3 py-1.5 text-xs font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-slate-800/50 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700/60 transition-colors cursor-pointer"
            >
              Reset
            </button>
          )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-2 border-t border-slate-100 dark:border-slate-800/60">
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1.5 pt-0.5 no-scrollbar sm:scrollbar-thin touch-pan-x flex-1">
          {GENERATIONS.map((gen) => (
            <button
              key={gen.id}
              onClick={() => onFilterChange({ generation: gen.id as any })}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition-all cursor-pointer shrink-0 active:scale-95 ${
        ...
        {onMarkRegionCaught && (
          <div className="flex items-center gap-1.5 shrink-0 pl-1">
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-950/50 dark:hover:bg-emerald-900/60 text-emerald-700 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-700/80 text-xs font-bold rounded-lg transition-all shadow-xs cursor-pointer whitespace-nowrap active:scale-95"
            ...
            <button
              ...
              className="p-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-500 hover:text-rose-600 dark:text-slate-400 dark:hover:text-rose-400 border border-slate-200 dark:border-slate-700 text-xs rounded-lg transition-all cursor-pointer shadow-xs active:scale-95"
