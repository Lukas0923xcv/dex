...
const isBaseForm = (p: Pokemon): boolean =>
  p.category === 'standard' || (!p.isForm && !p.isMega && !p.isCostume && !p.isGenderDifference);

function computeSizeRank(p: Pokemon): number {
...
function getSizeRank(p: Pokemon): number {
...
function computeInGameFormRank(p: Pokemon): number {
  const dex = p.dexNr;
  const fid = (p.formId || '').toUpperCase();
  const fname = (p.formName || '').toUpperCase();
  const name = (p.name || '').toUpperCase();
...
function getInGameFormRank(p: Pokemon): number {
